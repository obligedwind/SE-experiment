...
class node:
    ...
    def append(self, name: str, phone: int, t: bool, page: str = None, time: str = None):
        ...
        if (t):
            ...
            if (self.nc.fetchone() != None):
                sql = f"INSERT INTO u{phone}_to_merge (id, oprand, val_1, val_2) values ({phone}, 'teastu', '{time}', '{phone}_{self.phone}');"
            self.nc.execute(sql)
            self.ne.commit()
        else:
            ...
            if (self.nc.fetchone() != None):
                sql = f"INSERT INTO u{phone}_to_merge (id, oprand, val_1, val_2) values ({phone}, 'teastu', '{time}', '{self.phone}_{phone}');"
            self.nc.execute(sql)
            self.ne.commit()
    ...
    def drop(self, id_t: str, id_s: str):
        sql = f"SELECT * FROM global_relation WHERE id_t = '{id_t}' AND id_s = '{id_s}';"
        cursor.execute(sql)
        # 如果从全局关系表中没有相关记录，推测是个人的意淫，可以直接删除
        if cursor.fetchone() is None:
            sql = f"DELETE FROM u{self.phone}_relation WHERE id_t = '{id_t}' AND id_s = '{id_s}';"
            cursor.execute(sql)
            db.commit()
            return
...
def register(phone: int, psw: str, name: str):
    ...
    ne = psycopg2.connect(host='localhost',
                          ...
                          password=str(psw))
    rc = ne.cursor()
    sql = f"CREATE TABLE u{phone}_view (id VARCHAR(20) PRIMARY KEY, name VARCHAR(100), page VARCHAR(100), phone INT);"
    rc.execute(sql)
    ne.commit()
